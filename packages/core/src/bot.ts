...
/**
 * Computer player (5 difficulty levels). Pure policy: given a state and a seat,
 * pick one legal action — the host drives it through the *same* Arbiter path as
 * human seats, so a bot enjoys no structural advantage. Difficulty is expressed
 * on two axes:
...
export const BOT_LEVELS: readonly BotLevel[] = [1, 2, 3, 4, 5];
...
export const BOT_PROFILES: Record<BotLevel, BotProfile> = {
  1: { reactionMs: [700, 1100], skill: 0.30, planRefill: false, useBuffer: false, diligence: 0.55 },
  2: { reactionMs: [520, 820], skill: 0.50, planRefill: false, useBuffer: false, diligence: 0.75 },
  3: { reactionMs: [380, 600], skill: 0.68, planRefill: true, useBuffer: false, diligence: 0.9 },
  4: { reactionMs: [260, 430], skill: 0.85, planRefill: true, useBuffer: true, diligence: 1.0 },
  5: { reactionMs: [150, 280], skill: 1.0, planRefill: true, useBuffer: true, diligence: 1.0 },
};
...
export function chooseBotAction(
  ...
  // --- manual refill (uncovers quick cards; only mid/high levels plan it) ---
  ...
  // nothing placeable: in call mode, an empty quick pile means "call Stop and win"
  if (state.config.roundEndMode === 'call' && p.quick.length === 0)
