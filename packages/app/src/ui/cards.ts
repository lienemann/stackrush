 ...
 *
 * Signature element: the value appears twice, point-symmetric (readable from
 * any seat rotation), each numeral backed by its color's shape mark — the
 * shape does the colorblind work, no legend needed. 6 and 9 carry a baked-in
 * underline bar since cards are read upside down.
 *
...
/** one index: shape mark behind the numeral, optional 6/9 underline */
function indexGroup(card: Card, x: number, y: number, rot: number): string {
  const needsBar = card.value === 6 || card.value === 9;
  const bar = needsBar
    ? `<rect x="-13" y="17" width="26" height="4" rx="2" fill="#20242E"/>`
    : '';
  return `
    <g transform="translate(${x} ${y}) rotate(${rot})">
      <g fill="${CARD_COLORS[card.color]}" opacity="0.42">${shapeMark(card.color, 21)}</g>
      <text x="0" y="0" text-anchor="middle" dominant-baseline="central"
            class="card-value" fill="#20242E">${card.value}</text>
      ${bar}
    </g>`;
}
...
export function cardFaceSVG(card: Card, opts: { width?: number } = {}): string {
  ...
  const edge = CARD_COLORS[card.color];
  return `
  <svg viewBox="0 0 ${W} ${H}" width="${w}" height="${(w * H) / W}" class="card face"
       role="img" aria-label="${card.value}">
    <rect x="2" y="2" width="${W - 4}" height="${H - 4}" rx="11" fill="#F5F3EA"
          stroke="${edge}" stroke-width="4"/>
    ${indexGroup(card, 32, 40, 0)}
    ${indexGroup(card, W - 32, H - 40, 180)}
    <circle cx="${W - 15}" cy="15" r="5" fill="${OWNER_COLORS[card.owner] ?? '#666'}"/>
    <circle cx="15" cy="${H - 15}" r="5" fill="${OWNER_COLORS[card.owner] ?? '#666'}"/>
  </svg>`;
}
